...
async def read_settings(
    ...
    await log_event(db, LogLevel.DEBUG, f"使用者 {current_user.username} 嘗試獲取系統設定", "system_api", user_id=str(current_user.id), request_id=request_id_for_log)

    settings_response = await crud_settings.get_system_settings(db) # crud_settings.get_system_settings doesn't take current_user
    
    await log_event(db, LogLevel.DEBUG, f"成功為使用者 {current_user.username} 獲取系統設定", "system_api", user_id=str(current_user.id), request_id=request_id_for_log, details={"settings_retrieved": True})
...
async def update_settings_route(
    ...
    await log_event(db, LogLevel.INFO, f"使用者 {current_user.username} 嘗試更新系統設定", "system_api", user_id=str(current_user.id), request_id=request_id_for_log, details={"update_payload": log_payload})
    ...
    try:
        ...
        if not updated_settings_response:
            await log_event(db, LogLevel.ERROR, f"使用者 {current_user.username} 更新系統設定失敗 (CRUD 操作返回 None)", "system_api", user_id=str(current_user.id), request_id=request_id_for_log, details={"update_payload": log_payload})
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="更新設定失敗")
        
        await log_event(db, LogLevel.INFO, f"系統設定已成功被使用者 {current_user.username} 更新", "system_api", user_id=str(current_user.id), request_id=request_id_for_log, details={"updated_successfully": True})
        return updated_settings_response
    except Exception as e:
        await log_event(db, LogLevel.ERROR, f"使用者 {current_user.username} 更新系統設定時發生錯誤: {str(e)}", "system_api", user_id=str(current_user.id), request_id=request_id_for_log, details={"error": str(e), "update_payload": log_payload})
...
async def test_ai_api_key(
    ...
    await log_event(
        db, 
        LogLevel.INFO, 
        f"使用者 {current_user.username} 嘗試測試 Google AI API 金鑰 (結尾為 ...{payload.api_key[-4:] if len(payload.api_key) > 4 else '****'})", 
        "system_api_key_test", 
    ...
    await log_event(
        db, 
        log_level, 
        f"Google AI API 金鑰測試結果 for user {current_user.username}: {message}", 
        "system_api_key_test", 
        user_id=str(current_user.id),
        request_id=request_id_for_log, 
...
async def get_google_models_list(
    ...
    """
    request_id_for_log = request.state.request_id if hasattr(request.state, 'request_id') else None
    await log_event(db, LogLevel.DEBUG, f"使用者 {current_user.username} 請求 Google AI 模型列表", "system_api", user_id=str(current_user.id), request_id=request_id_for_log)
...
async def get_connection_info_route(
    ...
    (目前為模擬數據)
    """
    # Log user if necessary: await log_event(db, LogLevel.DEBUG, f"User {current_user.username} requested connection info", ...)
...
async def test_db_connection_endpoint(
    ...
    """
    # Log user activity: await log_event(db, LogLevel.INFO, f"User {current_user.username} initiated DB connection test", ...)
    try:
