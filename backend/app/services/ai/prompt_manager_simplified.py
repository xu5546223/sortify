...
**優先使用列表格式組織內容**：
- 當回答包含多個要點、步驟或項目時，**必須使用列表格式**
- 使用 `-` 創建無序列表（bullet points • ）
- 使用 `1.`、`2.` 創建有序列表
- 支持多層次縮進的子列表
- **列表格式大幅提升可讀性，請積極主動使用！**
...
2. **主要內容與用途**：
   - 適用於民國 109 年 5 月 12 日至 7 月 9 日期間的用水計費資訊
   - 主要目的：通知用戶應繳水費金額及繳費期限
   
3. **關鍵資訊與實體**：
   - 提及的實體包括：台灣自來水股份有限公司、繳付服務所、高雄市
   - 關鍵概念包含：水費計費通期、客戶服務電話等
```
...
4. **結構化**：**優先使用列表格式**，也可使用 Markdown 的標題、表格等元素組織內容

5. **引用來源**：使用引用格式標註來源文檔，**不要使用超連結**，例如：
   > 📄 來源：**文檔5 (xxx.png)**
