...
3. **聚焦性**（重要）：只回答用戶問題，只使用與問題直接相關的文檔內容
4. **結構化**：在 answer_text 中使用換行符和列表組織內容
5. **sources_used**：只列出實際用於回答的文檔
6. **置信度**：根據文檔內容的相關性評估 confidence_score（0-1）
7. **關鍵點**：提取3-5個關鍵點到 key_points 陣列

## ⛔ 禁止事項

1. **禁止提及無關文檔**：如果某些文檔與用戶問題無關，完全不要提及它們
2. **禁止說明無關內容**：不要說「雖然與問題無關，但文檔X提到了...」這類話
3. **禁止描述所有文檔**：不需要對每個提供的文檔都做說明
4. **禁止補充無關信息**：專注回答問題，不要添加額外的「順便一提」內容
...
4. **結構化**：優先使用列表格式組織多個要點，使用標題和表格增強可讀性
...
1. **禁止提及無關文檔**：如果某些文檔與用戶問題無關，**完全不要提及它們**
2. **禁止說明無關內容**：不要說「雖然與問題無關，但文檔X提到了...」「另外，文檔Y還包含...」這類話
3. **禁止描述所有文檔**：不需要對每個提供的文檔都做說明，只引用相關的
4. **禁止補充無關信息**：專注回答問題，不要添加額外的「順便一提」「此外還發現」等無關內容
5. **禁止羅列文檔內容**：不要以「文檔1包含...、文檔2包含...」的方式羅列所有文檔''',
        user_prompt_template='''問題：<user_question>{user_question}</user_question>
