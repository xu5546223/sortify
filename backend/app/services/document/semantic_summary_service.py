...
class SemanticSummaryService:
    ...
    async def _create_summary_vector(self, document: Document, semantic_summary: SemanticSummary) -> VectorRecord:
        ...
        if len(summary_text_for_vector) > max_embedding_length:
            ...
            # 智能壓縮各個組件
            compressed_filename = smart_truncate(filename, int(available_content_space * weights['filename']))
            compressed_summary = smart_truncate(summary_text, int(available_content_space * weights['summary']))
            compressed_key_terms = smart_compress_list(key_terms, int(available_content_space * weights['key_terms']))
            compressed_searchable = smart_compress_list(searchable_keywords, int(available_content_space * weights['searchable']))
            compressed_domains = smart_compress_list(knowledge_domains, int(available_content_space * weights['domains']))
            compressed_content_type = smart_truncate(content_type, int(available_content_space * weights['content_type']))
